...
public class LoopEvent implements ITransformable, Comparable<LoopEvent> {
	...
	/**
	 * Gets the earliest start time and the latest end time
	 * and puts them in a two-item array
	 */
	private int[] getMinMax(){
		if(resetCache || minmax == null){
			...
			for(TransformationEvent te : events){
				if(te.startTime < min) min = te.startTime;
				if(te.endTime > max) max = te.endTime;
			}
	...
	public static LoopEvent parse(String line){
		...
		for(String p : parts){
			if(p.isEmpty() || p.trim().equalsIgnoreCase("L")) continue;
			if(p1 == -1) p1 = Integer.parseInt(p.trim());
			if(p2 == -1) p2 = Integer.parseInt(p.trim());
		}
