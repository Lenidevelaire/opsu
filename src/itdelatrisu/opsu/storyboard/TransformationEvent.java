...
public class TransformationEvent implements Comparable<TransformationEvent> {
	...
	protected TransformationEvent(String transform, int easing, int start, int end, float[] startTimes, float[] endTimes){
	...
	public String toString() {
		...
		for(int i = 0; i < startValues.length; i++){
			...
			if(startVals[i] != endVals[i]) same = false;
		}
	...
	public static TransformationEvent[] parse(String s){
		...
		int valCount = (type == COLOR ? 3 : 
			(type == MOVE || type == VECTOR_SCALE ? 2 : 1));
		...
		if(count == 1){
			return new TransformationEvent[]{
					new TransformationEvent(type, easing, start, end, sv)
			};
		}
	 ...
	 */
	public static class Parameter extends TransformationEvent {
		public final String parameterType;
		
		private Parameter(int start, int end, String param){
